 ...
 */
public class FeCurrencyExecutor extends FeExecutorBase {

    public static final List<String> ALIASES = Arrays.asList("currency", "c");

    public static final String NAME = ALIASES.get(0);

    public static CommandSpec create(Map<List<String>, ? extends CommandCallable> children) {
        return CommandSpec.builder()
                .description(Text.of("Do things with currencies (lists subcommands)"))
                .permission(PluginInfo.DATA_ROOT + '.' + NAME + ".use")
                .children(children)
                .executor(new FeCurrencyExecutor())
                .build();
    }
